...
class OCScope {
public:
  typedef map<string, OCScope *> MapVariableDefinition;

  OCScope(OCNamedScopes *named_scopes);

  OCNamedScopes *get_named_scopes();

  void set_parent(OCScope *parent);
  OCScope *get_parent();
  ...
  OCDirectory *get_directory();
  void set_directory(OCDirectory *directory);

  string expand_string(const string &str);
  string expand_self_reference(const string &str, const string &varname);

  static void push_scope(OCScope *scope);
  static OCScope *pop_scope();
  static OCScope *get_bottom_scope();
  static OCScope *get_enclosing_scope(unsigned int n);
...
private:
  ...
  string expand_split(const string &params);
  ...
  string expand_formap(const string &params);
  string expand_relpath(const string &params);
  string expand_function(const string &funcname, const OCSubroutine *sub,
             const string &params);
  ...
  typedef vector<OCScope *> ScopeStack;
