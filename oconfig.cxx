...
usage() {
  cerr <<
    "\n"
    "oconfig [opts] subdir-name [subdir-name..]\n"
    "oconfig\n"
    ...
    "oconfig -s is a special form of the command that runs as a very limited\n"
    ...
    "  -h            Display this page.\n"
    "  -V            Report the version of oconfig, and exit.\n"
    "  -v            Turn on verbose output (may help in debugging .bld files).\n"
    "  -vv           Be very verbose (if you're getting desperate).\n"
    "  -P            Report the current platform name, and exit.\n\n"
    "  -D bld.dep    Examine the given dependency file, and re-run oconfig\n"
    ...
    "  -t <tag>      Time oconfig.  Dump profiling info when done. Optionally\n"
    "                print detail additional information for the tag specified.\n "
    "                The tag format is 'ROOT...' (gotten from a prior profiling run\n"
    "  -c config.bld Read the indicated user-level config.bld file after reading\n"
    "                the system config.bld file.  If this is omitted, the value\n"
    "                given in the environment variable OCONFIG_CONFIG is used\n"
    "                instead.\n\n";
}
...
static void
report_platform() {
  cerr << "oconfig built for default platform " << PLATFORM << ".\n";
}
...
main(int argc, char *argv[]) {
  ...
  extern char *optarg;
  extern int optind;
  const char *optstr = "hVvPD:drnNt::p:c:s:";
  ...
  string platform;
  char *platform_env = getenv("OCONFIG_PLATFORM");
  if(platform_env==NULL) 
     platform=PLATFORM;
    else platform=platform_env;

  string oconfig_config;
  bool got_oconfig_config = false;
  bool got_time_oconfig = false;
  string sed_command;
  ...
  while (flag != EOF) {
    ...
    case 't':
      // turn on time profiling, AND optionally profile the given tag
      // to print the line stack.  Profile_tag is part of ocProfileInfo
      got_time_oconfig = true;
      if ( optarg != NULL ) {
        profile_tag = optarg;
      }
      break;
    ...
    case 'c':
      oconfig_config = optarg;
      got_oconfig_config = true;
      break;
...
#ifdef WIN32_VC
  if (verbose_dry_run) {
    cerr << "Option -N treated like -n when oconfig is built without Cygwin.\n";
  }
#endif
  ...
  // defined in ocProfileInfo
  command_profile = new ocProfileInfo();
  if ( got_time_oconfig ) {
    command_profile->setCaptureState(1);
  }
  
  OCScope global_scope((OCNamedScopes *)NULL);
  global_scope.define_variable("OCONFIG", PACKAGE);
  global_scope.define_variable("OCONFIG_VERSION", VERSION);
  ...
  global_scope.define_variable("REDIRECT_FILENAME", REDIRECT_FILENAME);

  if (got_oconfig_config) {
    ...
    global_scope.define_variable("OCONFIG_CONFIG", oconfig_config);
  }
  ...
  OCMain ocmain(&global_scope);
  if (!ocmain.read_source(".")) {
    exit(1);
  }
  ...
  if (report_depends || report_reverse_depends) {
    ...
    for (int i = 1; i < argc; i++) {
      cerr << "\n";
      if (report_depends) {
        ocmain.report_depends(argv[i]);
      }
      cerr << "\n";
      if (report_reverse_depends) {
        ocmain.report_reverse_depends(argv[i]);
      }
    }
    ...
    if (argc < 2) {
      if (!ocmain.process_all()) {
        exit(1);
      }
    } else {
      for (int i = 1; i < argc; i++) {
        if (!ocmain.process(argv[i])) {
  ...
  if (got_time_oconfig) {
    cout << "time slice info" << endl << endl;
    command_profile->dumpTimesliceInfo();

    if ( ! profile_tag.empty() ) {
      cout << endl << "time slice detail" << endl << endl;
      command_profile->dumpTimesliceDetail();
    }
  }
