...
do_s_command(SedContext &context) {
  ...
  while (error == 0) {
        ...
        if (p < _string2.length()) {
          if (isdigit(_string2[p])) {
            ...
            if (ref <= 0 || ref >= (int)nmatch) {
              ...
              repl += string(str + static_cast<unsigned>(pmatch[ref].rm_so),
                             pmatch[ref].rm_eo - static_cast<unsigned>(pmatch[ref].rm_so));
            }
          } else {
    ...
    result += string(str, static_cast<unsigned>(pmatch[0].rm_so));
    result += repl;
    str += pmatch[0].rm_eo;
