...
// is available, because it allows us to access all the Cygwin
...
from_os_specific(const string &os_specific, Filename::Type type) {
  ...
  if (!panda_root.empty() && panda_root.length() < result.length()) {
    ...
    for (p = 0; p < panda_root.length() && matches; p++) {
      char c = static_cast<char>(tolower(panda_root[p]));
      if (c == '\\') {
        c = '/';
      }
  ...
  if (result.size() >= 3 && isalpha(result[0]) &&
      result[1] == ':' && result[2] == '/') {
    result[1] = static_cast<char>(tolower(result[0]));
    result[0] = '/';
  }
...
temporary(const string &dirname, const string &prefix, Type type) {
  if (dirname.empty()) {
    ...
    char *name = OCONFIG_TEMPNAM(NULL, prefix.c_str());
  ...
  do {
    ...
    time_t hash = (clock() * time(NULL)) & 0xffffff;
    char hex_code[10];
    sprintf(hex_code, "%06x", static_cast<unsigned>(hash));
...
set_dirname(const string &s) {
  if (s.empty()) {
    ...
    int length_change = static_cast<int>(ss.length() - _basename_start);
...
set_fullpath_wo_extension(const string &s) {
  int length_change = static_cast<int>(s.length() - _basename_end);
...
set_basename_wo_extension(const string &s) {
  int length_change = static_cast<int>(s.length() - (_basename_end - _basename_start));
...
exists() const {
  ...
  DWORD results = GetFileAttributesA(os_specific.c_str());
  if (results != -1) {
    exists = true;
  }
...
////////////////////////////////////////////////////////////////////
bool Filename::
resolve_filename(const OCSearchPath &searchpath,
...
////////////////////////////////////////////////////////////////////
int Filename::
find_on_searchpath(const OCSearchPath &searchpath) {
  if (_filename.empty() || _filename[0] != '/') {
    return -1;
  }
...
unlink() const {
  string os_specific = to_os_specific();
  return (::OCONFIG_UNLINK(os_specific.c_str()) == 0);
}
...
make_dir() const {
  ...
  while (slash != string::npos) {
    if (slash != 0) {
      Filename component(dirname.substr(0, slash+1), "foo");
      string os_specific = component.to_os_specific();
      os_specific = os_specific.substr(0, os_specific.length()-4);
#ifndef WIN32_VC
      mkdir(os_specific.c_str(), 0777);
#else
      if ((os_specific.length()!=2)||(os_specific[1]!=':')) {
...
r_make_canonical(const Filename &cwd) {
  ...
  if (OCONFIG_CHDIR(os_specific.c_str()) >= 0) {
